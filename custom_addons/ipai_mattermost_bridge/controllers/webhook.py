...
def _auth_ok(req):
    secret_param = (
        ...
        .sudo()
        .get_param("ipai.mm_webhook_secret", default="")
    )
...
def _log_event(source: str, payload: dict):
    ...
    message = json.dumps(
        {"source": source, "keys": list(payload.keys())}, ensure_ascii=False
    )[:1000]
