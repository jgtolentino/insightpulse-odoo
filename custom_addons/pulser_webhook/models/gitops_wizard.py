...
class PulserGitOpsWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if "message" in fields_list and not res.get("message"):
            ...
            res["message"] = (
                f"chore(gitops): dispatch from {model or 'manual'} #{rec_id or ''} {name}".strip()
            )
    ...
    def action_dispatch(self):
        ...
        base_url = (
            ...
            .sudo()
            .get_param("web.base.url", "http://127.0.0.1:8069")
            .rstrip("/")
        )
        ...
        try:
            ...
            if not ok:
                ...
                raise UserError(
                    _("Dispatch failed (HTTP %s). See response field for details.")
                    % r.status_code
                )
