...
class IpCashAdvance(models.Model):
    ...
    name = fields.Char(
        required=True,
        default=lambda self: self.env["ir.sequence"].next_by_code("ip.cash.advance")
        or "Cash Advance",
    )
    ...
    related_expense_ids = fields.One2many(
        "hr.expense", "cash_advance_id", string="Linked Expenses"
    )
