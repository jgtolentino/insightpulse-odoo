...
class IpTeDashboard(models.TransientModel):
    ...
    def _compute_kpis(self):
        ...
        for rec in self:
            rec.pending_approvals = Expense.search_count(
                [("state", "in", ["reported", "submitted"])]
            )
            rec.ocr_queue = Expense.search_count(
                [("ocr_status", "in", ["new", "queued"])]
            )
            rec.open_cash_advances = CA.search_count(
                [("state", "in", ["approved", "released"])]
            )
            rec.overdue_liquidations = Liquid.search_count(
