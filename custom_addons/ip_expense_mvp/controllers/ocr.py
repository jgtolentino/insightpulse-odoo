...
class IpExpenseController(http.Controller):
    ...
    @http.route("/ip/mobile/receipt", type="http", auth="user", website=True, methods=["GET","POST"])
    def mobile_receipt(self, **kw):
        ...
        # create expense
        exp = request.env["hr.expense"].sudo().create({
        ...
        try:
            ...
            pass
        return werkzeug.utils.redirect(f"/web#id={exp.id}&model=hr.expense&view_type=form")
