...
class HROffboarding(models.Model):
    ...
    exit_date = fields.Date(string="Last Working Day", required=True, tracking=True)

    exit_reason = fields.Text(string="Reason for Leaving", tracking=True)
    ...
    checklist_ids = fields.One2many(
        "hr.offboarding.checklist", "offboarding_id", string="Clearance Checklist"
    )

    final_pay_id = fields.Many2one(
        "hr.final.pay.computation", string="Final Pay Computation", readonly=True
    )
