    ...
    def upload_receipt(self, **kw):
        ...
        if not f:
            return request.make_response(
                "No file provided", headers=[("Status", "400 Bad Request")]
            )

        ai_url = _cfg("ip.ai_ocr_url", "https://ocr.insightpulseai.net/v1/ocr/receipt")
        try:
            r = requests.post(
                ai_url, files={"file": (f.filename, f.stream, f.mimetype)}, timeout=60
            )
            r.raise_for_status()
        except Exception as e:
            return request.make_response(
                f"OCR error: {e}", headers=[("Status", "502 Bad Gateway")]
            )

        rec = (
            request.env["ip.ocr.receipt"]
            .sudo()
            .create(
                {
                    "name": f.filename,
