...
class MCPOperation(models.Model):
    ...
    server_id = fields.Many2one('mcp.server', string='MCP Server', required=True, ondelete='cascade')
    ...
    def execute(self):
        ...
        try:
            ...
            if 'error' in result_data:
                self.write({
                    'state': 'failed',
                    'error_message': result_data['error'].get('message', 'Unknown error'),
