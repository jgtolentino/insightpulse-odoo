...
class IPAIAgentController(http.Controller):
    ...
    @http.route(
        "/ipai/agent/webhook", type="json", auth="public", methods=["POST"], csrf=False
    )
    def agent_webhook(self, **kwargs):
        ...
        config_api_key = (
            ...
            .sudo()
            .get_param("ipai_agent.webhook_api_key")
        )
        ...
        user = (
            ...
            .sudo()
            .search([("email", "=", kwargs.get("user_email"))], limit=1)
        )
        ...
        context = {
            ...
            "timestamp": request.env["mail.message"]
            .sudo()
            .fields.Datetime.now()
            .isoformat(),
        }
