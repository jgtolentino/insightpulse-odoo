...
class MailMessage(models.Model):
    ...
    is_agent_query = fields.Boolean(
        string='AI Agent Query',
        default=False,
        help='Message was processed by AI agent'
    )
    agent_response_id = fields.Many2one(
        'mail.message',
        string='Agent Response',
        help='AI agent response to this query'
    )
    ...
    def _build_agent_context(self, message):
        ...
        """
        user = message.author_id.user_ids[0] if message.author_id.user_ids else self.env.user
    ...
    def _post_agent_response(self, original_message, response):
        ...
        bot_user = self.env.ref('ipai_agent.user_agent_bot', raise_if_not_found=False)
        if not bot_user:
            raise UserError(_('AI Agent bot user not configured. Please install ipai_agent data.'))
