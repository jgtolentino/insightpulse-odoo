        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    _logger.warning(
                        f"Agent API timeout (attempt {attempt + 1}/{max_retries + 1})"
                    )
                    continue
                raise UserError(
                    _("Agent API timeout after %d attempts") % (max_retries + 1)
                )

            except requests.exceptions.RequestException as e:
                if attempt < max_retries:
                    _logger.warning(
                        f"Agent API error (attempt {attempt + 1}/{max_retries + 1}): {str(e)}"
                    )
                    continue
