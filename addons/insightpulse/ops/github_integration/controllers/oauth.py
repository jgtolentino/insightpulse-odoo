...
class GitHubOAuthController(http.Controller):
    ...
    @http.route(
        "/odoo/github/auth/callback", type="http", auth="public", methods=["GET"]
    )
    def github_oauth_callback(self, code=None, state=None, error=None, **kwargs):
        ...
        if error:
            ...
            return request.render(
                "github_integration.oauth_error",
                {
                    "error": error,
        ...
        try:
            ...
            client_id = (
                request.env["ir.config_parameter"].sudo().get_param("github.client_id")
            )
            client_secret = (
                request.env["ir.config_parameter"]
                .sudo()
                .get_param("github.client_secret")
            )
            ...
            if token_response.status_code == 200:
                ...
                return request.render(
                    "github_integration.oauth_success",
                    {
