...
class IrModuleModule(models.Model):
    ...
    ], string='Source', compute='_compute_module_source', store=True, search='_search_module_source')
    
    source_repository = fields.Char(string='Repository URL', compute='_compute_source_repository')
    source_path = fields.Char(string='File Path', compute='_compute_source_path')
    is_oca_module = fields.Boolean(string='Is OCA Module', compute='_compute_module_source', store=True)
    ...
    def _compute_module_source(self):
        ...
        for module in self:
            ...
            elif any(x in module_path for x in ['/custom', '/insightpulse', '/bir-forms', '/document-ai']):
    ...
    def _compute_source_repository(self):
        ...
        for module in self:
            if module.module_source == 'odoo_community':
                module.source_repository = f'https://github.com/odoo/odoo/tree/19.0/addons/{module.name}'
