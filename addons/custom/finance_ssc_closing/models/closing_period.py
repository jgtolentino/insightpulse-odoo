...
class FinanceClosingPeriod(models.Model):
    ...
    bir_task_ids = fields.One2many(
        ...
        'period_id',
        string='BIR Compliance Tasks'
    )
    bir_all_filed = fields.Boolean(
        string='All BIR Forms Filed',
        compute='_compute_bir_status',
        store=True
    )
    ...
    def _compute_task_statistics(self):
        ...
        for period in self:
            ...
            period.task_completed_count = len(tasks.filtered(lambda t: t.state == 'completed'))
            period.task_pending_count = len(tasks.filtered(lambda t: t.state in ('pending', 'in_progress')))

            if period.task_count > 0:
                period.completion_percentage = (period.task_completed_count / period.task_count) * 100
            else:
    ...
    def action_start_closing(self):
        ...
        for period in self:
            ...
            if not period.task_ids:
                raise UserError(_('Please create closing tasks before starting the closing process.'))
    ...
    def action_submit_for_review(self):
        ...
        for period in self:
            if period.state != 'in_progress':
                raise UserError(_('Only in-progress periods can be submitted for review.'))
            if period.completion_percentage < 100:
                raise UserError(_('All tasks must be completed before submitting for review.'))
    ...
    def _cron_send_deadline_reminders(self):
        ...
        for period in periods:
            ...
            if days_remaining in (7, 3, 1):  # Send reminders at 7, 3, and 1 day before deadline
                period.message_post(
                    body=_('Reminder: %d days remaining until closing deadline.') % days_remaining,
