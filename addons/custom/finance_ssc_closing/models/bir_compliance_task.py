...
class FinanceBIRComplianceTask(models.Model):
    ...
    bir_form = fields.Selection(
        [
            ...
            (
                "1601e",
                "Form 1601-E - Quarterly Remittance Return of Creditable Income Taxes",
            ),
            ("1601f", "Form 1601-F - Monthly Remittance Return of Final Income Taxes"),
            (
                "1602",
                "Form 1602 - Monthly Remittance Return of Final Withholding Taxes",
            ),
            ...
            (
                "1604e",
                "Form 1604-E - Annual Information Return of Creditable Income Taxes Withheld",
            ),
            (
                "1604f",
                "Form 1604-F - Annual Information Return of Final Income Taxes Withheld",
            ),
    ...
    due_date = fields.Date(
        string="Due Date", required=True, tracking=True, help="BIR filing deadline"
    )
    filing_date = fields.Date(
        string="Filing Date", tracking=True, help="Actual date filed with BIR"
    )
    ...
    filed_by = fields.Many2one("res.users", string="Filed By", tracking=True)

    # Tax Details
    tax_amount = fields.Monetary(
        string="Tax Amount", currency_field="currency_id", tracking=True
    )
    penalty_amount = fields.Monetary(
        string="Penalty Amount", currency_field="currency_id", tracking=True
    )
    ...
    days_overdue = fields.Integer(
        string="Days Overdue", compute="_compute_overdue_status", store=True
    )
    ...
    def _compute_name(self):
        """Generate task name"""
        for task in self:
            bir_form_name = dict(self._fields["bir_form"].selection).get(
                task.bir_form, ""
            )
            task.name = (
                f"{bir_form_name} - {task.period_id.name} - {task.company_id.name}"
            )
