    ...
    def health_check(self):
        ...
        if config.ocr_endpoint:
            try:
                ...
                components['ocr']['status'] = 'healthy' if response.status_code == 200 else 'unhealthy'
        ...
        if config.llm_endpoint:
            try:
                ...
                response = requests.get(
                    ...
                    timeout=5,
                    headers={'Authorization': f'Bearer {config.llm_token}'} if config.llm_token else {}
                )
                components['llm']['response_time'] = time.time() - llm_start
                components['llm']['status'] = 'healthy' if response.status_code == 200 else 'unhealthy'
        ...
        if config.agent_endpoint:
            try:
                ...
                response = requests.get(
                    ...
                    timeout=5,
                    headers={'Authorization': f'Bearer {config.agent_token}'} if config.agent_token else {}
                )
                components['agent']['response_time'] = time.time() - agent_start
                components['agent']['status'] = 'healthy' if response.status_code == 200 else 'unhealthy'
        ...
        _logger.info(f"Microservices health check completed in {total_time:.2f}s: {components}")
