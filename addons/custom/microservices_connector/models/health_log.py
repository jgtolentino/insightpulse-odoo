...
class MicroservicesHealthLog(models.Model):
    ...
    component = fields.Selection(
        [("ocr", "OCR Service"), ("llm", "LLM Service"), ("agent", "Agent Service")],
        string="Component",
        required=True,
    )
    ...
    response_time = fields.Float(
        string="Response Time (s)", help="Response time in seconds"
    )
    error_message = fields.Text(
        string="Error Message", help="Detailed error message if any"
    )
    total_check_time = fields.Float(
        string="Total Check Time (s)", help="Total time taken for the health check"
    )
    create_date = fields.Datetime(string="Check Date", default=fields.Datetime.now)
