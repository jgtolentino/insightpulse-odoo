...
class SecurityHardeningController(http.Controller):
    ...
    def block_database_manager(self, **kwargs):
        ...
        _logger.warning("Database manager access attempt blocked from IP: %s", request.httprequest.remote_addr)
    ...
    @http.route('/web/database/selector', type='http', auth='public', methods=['GET', 'POST'])
    def block_database_selector(self, **kwargs):
        """Block database selector access in production"""
        _logger.warning("Database selector access attempt blocked from IP: %s", request.httprequest.remote_addr)
