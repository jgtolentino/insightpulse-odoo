...
class IpaiVendorScore(models.Model):
    ...
    vendor_id = fields.Many2one("res.partner", required=True, domain=[("supplier_rank", ">", 0)], ondelete="cascade")
    ...
    def _compute_avg(self):
        for rec in self:
            rec.score_avg = (rec.score_quality + rec.score_on_time + rec.score_cost) / 3.0 if any([
                rec.score_quality, rec.score_on_time, rec.score_cost]) else 0.0
