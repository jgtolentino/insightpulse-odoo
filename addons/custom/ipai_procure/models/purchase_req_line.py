...
class IpaiPurchaseReqLine(models.Model):
    ...
    requisition_id = fields.Many2one(
        "ipai.purchase.requisition", required=True, ondelete="cascade"
    )
    product_id = fields.Many2one("product.product", string="Product")
    name = fields.Char(string="Description")
    qty = fields.Float(default=1.0)
    uom_id = fields.Many2one(
        "uom.uom", string="UoM", related="product_id.uom_id", store=True, readonly=False
    )
    price_unit = fields.Monetary(currency_field="currency_id", default=0.0)
    currency_id = fields.Many2one(
        "res.currency", related="requisition_id.currency_id", store=True
    )
