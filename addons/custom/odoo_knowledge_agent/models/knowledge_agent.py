            ...
            if result.returncode == 0:
                ...
                if output_file.exists():
                    with open(output_file, "r") as f:
                        ...
                        pages_count = (
                            max([issue.get("page", 0) for issue in data]) if data else 0
                        )
                    ...
                    _logger.info(
                        f"✅ Forum scraping completed: {issues_count} issues from {pages_count} pages"
                    )
                else:
                ...
                raise Exception(
                    f"Scraper failed with code {result.returncode}: {error_msg}"
                )
...
class KnowledgeAgentLog(models.Model):
    ...
    agent_id = fields.Many2one(
        "odoo.knowledge.agent", string="Agent", required=True, ondelete="cascade"
    )
