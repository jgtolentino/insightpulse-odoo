...
class TestSupersetToken(TransactionCase):
    ...
    def test_token_creation(self):
        ...
        self.assertEqual(len(token.token), 43, "Token should be 43 characters (base64url)")
    ...
    def test_token_expiry(self):
        ...
        self.assertLess(time_diff, 60, "Expiry should be approximately 24 hours from creation")
    ...
    def test_get_or_create_token_force_new(self):
        ...
        self.assertNotEqual(token1.id, token2.id, "Should create new token when force_new=True")
    ...
    def test_cleanup_expired_tokens(self):
        ...
        self.assertTrue(expired_token.is_active, "Expired token should be active before cleanup")
        self.assertTrue(valid_token.is_active, "Valid token should be active")

        # Run cleanup
        self.env['superset.token'].cleanup_expired_tokens()

        self.assertFalse(expired_token.is_active, "Expired token should be inactive after cleanup")
    ...
    def test_token_stats(self):
        ...
        self.assertGreaterEqual(stats['total_tokens'], 3, "Should have at least 3 tokens")
        self.assertGreaterEqual(stats['active_tokens'], 3, "Should have at least 3 active tokens")
        self.assertIsInstance(stats['most_used_tokens'], list, "Should return list of most used tokens")
