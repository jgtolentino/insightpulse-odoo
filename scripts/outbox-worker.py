...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("/tmp/outbox-worker.log")],
)
...
class OutboxWorker:
    ...
    def fetch_batch(self) -> List[OutboxRecord]:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                ...
                """
                cur.execute(
                    query, (self.worker_id, LOCK_TIMEOUT_MINUTES, self.batch_size)
                )
                rows = cur.fetchall()
                conn.commit()
    ...
    def process_record(self, record: OutboxRecord) -> bool:
        ...
        try:
            logger.info(
                f"Processing record {record.id}: {record.operation} on {record.model}"
            )

            if self.dry_run:
                logger.info(
                    f"[DRY RUN] Would {record.operation} {record.model}: {record.payload}"
                )
                return True
            ...
            if record.operation == "upsert":
                ...
                if not record_id:
                    raise ValueError(
                        "Delete operation requires 'odoo_id' or 'id' in payload"
                    )
            ...
            logger.error(
                f"Error processing record {record.id}: {str(e)}", exc_info=True
            )
    ...
    def _mark_failed(self, record_id: int, error: str, attempts: int):
        ...
        if attempts >= MAX_RETRIES:
            status = "failed"
            logger.warning(
                f"Record {record_id} permanently failed after {attempts} attempts"
            )
        else:
            status = "queued"  # Retry
            logger.info(
                f"Record {record_id} will retry after {backoff}s (attempt {attempts})"
            )
    ...
    def run_daemon(self):
        ...
        while self.running:
            try:
                ...
                logger.error(
                    f"Unexpected error in daemon loop: {str(e)}", exc_info=True
                )
                time.sleep(POLL_INTERVAL_SECONDS)
...
def main():
    ...
    parser.add_argument(
        "--once", action="store_true", help="Process queue once and exit"
    )
    parser.add_argument(
        "--daemon", action="store_true", help="Run continuously (production mode)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview changes without writing to Odoo"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
