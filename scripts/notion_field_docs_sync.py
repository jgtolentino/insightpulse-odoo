...
class FieldDocsNotionSync:
    ...
    def generate_external_id(self, field: dict) -> str:
        ...
        module = Path(field['file']).parts[1] if len(Path(field['file']).parts) > 1 else 'unknown'
    ...
    def sync_field(self, field: dict) -> str:
        ...
        properties = {
            'Field Name': create_notion_property('title', f"{field['model']}.{field['field_name']}"),
            'Model': create_notion_property('rich_text', field['model']),
            'Field Type': create_notion_property('select', field['field_type']),
            'Label': create_notion_property('rich_text', field['params'].get('label', '')),
            'Help Text': create_notion_property('rich_text', field['params'].get('help', '')),
            'Required': create_notion_property('checkbox', field['params'].get('required', False)),
            'Readonly': create_notion_property('checkbox', field['params'].get('readonly', False)),
...
def main():
    ...
    parser.add_argument('--database-id', help='Notion database ID (or use NOTION_FIELDS_DB_ID)')
