...
def update_error_history(
    ...
    print(f"   Auto-fix success rate: {sum(1 for e in history if e.get('auto_fixed') and e.get('success')) / max(sum(1 for e in history if e.get('auto_fixed')), 1):.1%}")
...
def main():
    ...
    parser.add_argument('--success', required=True, type=lambda x: x.lower() == 'true',
