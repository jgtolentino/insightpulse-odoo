    ...
    def fetch_documentation(
        ...
        if current_depth < max_depth:
            for link in main_content.find_all("a", href=True):
                ...
                if (
                    "odoo.com/documentation" in full_url
                    and full_url not in self.visited_urls
                ):
                    sections.extend(
                        self.fetch_documentation(full_url, max_depth, current_depth + 1)
                    )

        return sections

    def chunk_content(
        self, content: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP
    ) -> List[str]:
        """
