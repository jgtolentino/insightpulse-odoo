        ...
        for part in version_parts:
            if part.replace(".", "").isdigit():
                ...
                if major_version >= int(REQUIRED_POSTGRES_VERSION):
                    return True, f"PostgreSQL {part} ✓"
                else:
                    return (
                        False,
                        f"PostgreSQL {REQUIRED_POSTGRES_VERSION}+ required, found {part}",
                    )
...
def scan_addon_dependencies(addons_path: Path) -> Dict[str, List[str]]:
    ...
    for addon_dir in addons_path.rglob("__manifest__.py"):
        ...
        try:
            ...
            print(f"⚠️  Error reading {addon_name}: {e}")
...
def generate_requirements_txt(dependencies: Dict[str, List[str]], output_path: Path):
    ...
    print(f"✓ Generated {output_path} with {len(all_packages)} packages")
    ...
    for manifest_path in addons_path.rglob("__manifest__.py"):
        ...
        try:
            ...
            if "license" not in manifest:
                ...
                changes_made = True
                print(f"  → Added missing license to {addon_name}")
            ...
            print(f"⚠️  Error processing {addon_name}: {e}")
...
def main():
    ...
    print("🔧 Odoo Deployment Auto-Fix Utility")
    print("=" * 50)
    print()
    ...
    if not python_ok:
        print("❌ CRITICAL: Incompatible Python version")
        sys.exit(1)
    ...
    if missing_packages:
        ...
        print("✓ All required system packages installed")

    # Scan addon dependencies
    addons_path = Path("addons")

    if not addons_path.exists():
        print("\n⚠️  No 'addons' directory found")
        return
    ...
    if cycles:
        print(f"⚠️  Circular dependencies detected in: {', '.join(cycles)}")
