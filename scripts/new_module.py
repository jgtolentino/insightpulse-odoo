...
if len(sys.argv)<2: raise SystemExit("usage: scripts/new_module.py <snake_name>")
name=sys.argv[1]; root=pathlib.Path(__file__).resolve().parent.parent
...
""").strip()+"\n")
(mod/"views"/f"{name}_views.xml").write_text(textwrap.dedent(f"""<?xml version="1.0" encoding="utf-8"?>
<odoo>
