...
class RepoStructureValidator:
    ...
    def check_forbidden_patterns(self):
        ...
        for pattern in self.REQUIRED_STRUCTURE['forbidden_patterns']:
            ...
            if matches:
                ...
                self.errors.append(f"Found forbidden pattern: {pattern} ({len(matches)} matches)")
    ...
    def validate_file_contents(self):
        ...
        yaml_files = [f for f in yaml_files if not any(p in str(f) for p in ['addons/', 'venv/', '.venv/'])]
        yaml_valid = 0
        ...
        for yaml_file in yaml_files[:50]:  # Limit to first 50 to avoid slowdown
            try:
                ...
                self.errors.append(f"Invalid YAML: {yaml_file.relative_to(self.repo_path)} - {str(e)}")

        print(f"   YAML: {yaml_valid}/{len(yaml_files[:50])} valid")

        # JSON files
        json_files = list(self.repo_path.glob('**/*.json'))
        json_files = [f for f in json_files if not any(p in str(f) for p in ['addons/', 'venv/', '.venv/', 'node_modules/'])]
        json_valid = 0
        ...
        for json_file in json_files[:50]:  # Limit to first 50
            try:
                ...
                self.errors.append(f"Invalid JSON: {json_file.relative_to(self.repo_path)} - {str(e)}")

        print(f"   JSON: {json_valid}/{len(json_files[:50])} valid")

        # Python files (in scripts/ and tests/ only)
        python_files = list(self.repo_path.glob('scripts/**/*.py')) + list(self.repo_path.glob('tests/**/*.py'))
        python_valid = 0
        ...
        for py_file in python_files:
            try:
                ...
                self.errors.append(f"Invalid Python: {py_file.relative_to(self.repo_path)} - {str(e)}")
        ...
        try:
            ...
            for section in required_sections:
                if section not in content:
                    self.warnings.append(f"Skill missing section '{section}': {skill_file.relative_to(self.repo_path)}")
                    return False

            # Check for YAML frontmatter or title
            if not content.startswith('---') and not content.startswith('# '):
                self.warnings.append(f"Skill has no frontmatter or title: {skill_file.relative_to(self.repo_path)}")
                return False
    ...
    def check_cross_references(self):
        ...
        if makefile.exists():
            ...
            scripts = re.findall(r'(?:bash|sh|python3?|\./)?\s*scripts/([a-z0-9_/-]+\.(?:sh|py))', content, re.IGNORECASE)

            missing_scripts = []
            for script in scripts:
                if not (self.repo_path / 'scripts' / script).exists():
                    missing_scripts.append(script)
                    self.warnings.append(f"Makefile references missing script: scripts/{script}")
