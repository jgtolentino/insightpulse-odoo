...
class OCAVendor:
    ...
    def update_index(self):
        ...
        print(f"Index updated: {len(updated_repos)} repositories, {total_modules} modules")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if result.returncode != 0:
                    print(f"  Warning: Could not clone {repo_name} (branch {self.version})")
    ...
    def search_modules(self, query: str) -> List[Dict]:
        ...
        for repo in self.index.get('repositories', []):
            for module in repo.get('modules', []):
                if (query_lower in module.get('name', '').lower() or
                    query_lower in module.get('summary', '').lower() or
                    query_lower in module.get('display_name', '').lower()):
    ...
    def check_updates(self, addon_path: str):
        ...
        if updates_available:
            ...
            for update in updates_available:
                print(f"  - {update['module']}: {update['vendored_version']} → {update['current_version']}")
        else:
...
def main():
    ...
    parser.add_argument('--update-index', action='store_true', help='Update OCA module index')
    ...
    parser.add_argument('--check-updates', type=str, help='Check for updates in addon path')
