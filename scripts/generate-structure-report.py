...
class StructureHealthReport:
    ...
    def check_structure_compliance(self) -> dict:
        ...
        return {
            ...
            'overall_percentage': ((existing_dirs + existing_files) / (len(required_dirs) + len(required_files)) * 100)
        }

    def check_documentation_coverage(self) -> dict:
        """Check documentation completeness."""

        major_dirs = ['skills', 'prompts', 'evals', 'tests', 'monitoring', 'custom', 'infrastructure', 'scripts']
        ...
        dirs_with_readme = sum(1 for d in existing_major_dirs if (self.repo_path / d / 'README.md').exists())

        # Count Python files with docstrings
        py_files = list(self.repo_path.glob('scripts/**/*.py')) + list(self.repo_path.glob('tests/**/*.py'))
        ...
        return {
            'readme_coverage': (dirs_with_readme / len(existing_major_dirs) * 100) if existing_major_dirs else 0,
            'code_documentation': (documented_py_files / total_py_files * 100) if total_py_files > 0 else 0,
    ...
    def check_skill_maturity(self) -> dict:
        ...
        for skill_file in skill_files:
            try:
                ...
                has_competencies = '## Core Competencies' in content or '## Purpose' in content
                has_validation = '## Validation' in content or '## Success Metrics' in content or len(content) > 500
        ...
        return {
            ...
            'maturity_rate': (mature_skills / len(skill_files) * 100) if skill_files else 0
        }
    ...
    def check_automation_level(self) -> dict:
        ...
        github_workflows = len(list(workflows_dir.glob('*.yml'))) if workflows_dir.exists() else 0
        ...
        if makefile.exists():
            try:
                with open(makefile) as f:
                    makefile_targets = len([l for l in f if l.strip() and ':' in l and not l.startswith('#') and not l.startswith('\t')])
            except:
                pass
        ...
        automation_score = min(100, (github_workflows * 10 + makefile_targets * 2 + scripts * 3))
    ...
    def check_cicd_health(self) -> dict:
        ...
        existing = sum(1 for w in workflows if any(r in w.stem.lower() for r in required_workflows))

        health = 'good' if existing >= len(required_workflows) else 'needs_work' if existing >= 2 else 'poor'
...
def main():
    ...
    print(f"Overall Score: {report['scores']['overall']:.1f}% (Grade: {report['scores']['grade']})")
    print()
    ...
    print(f"    • README coverage: {report['metrics']['documentation_coverage']['readme_coverage']:.1f}%")
    print(f"    • Code documentation: {report['metrics']['documentation_coverage']['code_documentation']:.1f}%")
    print(f"    • Total MD files: {report['metrics']['documentation_coverage']['total_md_files']}")
    ...
    print(f"    • Mature skills: {report['metrics']['skill_maturity']['mature_skills']}")

    print(f"  Automation:")
    print(f"    • GitHub workflows: {report['metrics']['automation_level']['github_workflows']}")
    print(f"    • Makefile targets: {report['metrics']['automation_level']['makefile_targets']}")
    print(f"    • Automation scripts: {report['metrics']['automation_level']['automation_scripts']}")
    print()
